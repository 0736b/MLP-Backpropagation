...
def plt_flood_lastepoch(result_trained, result_tested, model):
    ...
    test_name = ['Train'] * 10
    train_name = ['Test'] * 10
    new_idx = [str(i) for i in range(0,20,1)]
    for i in range(0, 10,1):
        trained_l.append(result_trained[i][len(result_trained[i]) - 1])
        idx.append(str(i+1))
    er_train_l = pd.DataFrame(trained_l, columns=['RMSE'])
    er_train_n = pd.DataFrame(train_name, columns=['Test/Train'])
    df_idx = pd.DataFrame(idx, columns=['Iteration'])
    # er_train_l.index.name = 'iteration'
    # er_train_n.index.name = 'iteration'
    merge_train = pd.concat([df_idx,er_train_l, er_train_n], axis=1)
    
    er_tested_l = pd.DataFrame(result_tested, columns=['RMSE'])
    er_tested_n = pd.DataFrame(test_name, columns=['Test/Train'])
    # er_tested_l.index.name = 'iteration'
    # er_tested_n.index.name = 'iteration'
    merge_tested = pd.concat([df_idx, er_tested_l, er_tested_n], axis=1)
    
    merge_er = pd.concat([merge_train, merge_tested])
    # print(merge_er)
    sns.barplot(x='Iteration', y='RMSE', data=merge_er, hue='Test/Train')
    plt.title('Flood model ' + model + ' Train and test RMSE', fontweight='bold', fontsize=24)
    plt.ylabel('RMSE')
    plt.show()
